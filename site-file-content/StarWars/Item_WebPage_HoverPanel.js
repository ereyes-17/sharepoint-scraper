...
,'                <div id="', $htmlEncode(id + HP.ids.body) ,'" class="ms-srch-hover-body">'
                    ...
                    if(!Srch.U.w(ctx.CurrentItem.csr_Path) && Srch.U.isSameHost(ctx.CurrentItem.csr_Path, Srch.U.getHostName())) {
						...
						var encodedSrcOfIframe = $urlHtmlEncodeString(ctx.CurrentItem.csr_Path);
ms_outHtml.push('				    '
...
,'                            <iframe id="',idViewerEncoded,'" src="', encodedSrcOfIframe ,'" scrolling="no" frameborder="0px" class="ms-srch-hover-siteViewer">'
,'                            </iframe>                            '
,'                        </div>'
);	                                               
                    }
...
,'                        </div>'
                        ...
                        {ULSSHg:;
                            ...
                            Srch.SiteSearchUtil.populateSiteSearchResults(path, siteResultsId, query, id + HP.ids.noData, timeZoneId);
                        });
                    } 
ms_outHtml.push('	'
,'                    ', ctx.RenderBody(ctx) ,'					'
